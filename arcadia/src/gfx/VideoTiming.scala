...
class VideoTiming(config: VideoTimingConfig) extends Module {
  ...
  // Offset the position vector so the display region begins at the origin (i.e. (0, 0)), and apply
  // any manual offset.
  val pos = UVec2(x - hBeginDisplay + config.hOffset.U, y - vBeginDisplay + config.vOffset.U)
